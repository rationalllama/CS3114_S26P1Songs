...
public class SongsTest extends TestCase {
    ...
    public void testInsertBasic() throws Exception {
        ...
        assertFuzzyEquals("", it.insert("A", "S")); // artist = "A", song = "S"
        assertFuzzyEquals("", it.insert("B", "T"));

        String artists = it.print("artist");
        assertTrue(artists.contains("|A|"));
        assertTrue(artists.contains("|B|"));

        String songs = it.print("song");
        assertTrue(songs.contains("|S|"));
        assertTrue(songs.contains("|T|"));
    }
     ...
     */
    public void testDuplicateArtist() throws Exception {
        it = new SongsDB();
        it.create(10, 32);

        assertFuzzyEquals("", it.insert("A", "S"));
        assertFuzzyEquals(
    ...
    /**
     * Quadratic probing collision test
     */
    public void testQuadraticProbing() throws Exception {
        it = new SongsDB();
        it.create(5, 32); // small table = collisions likely

        it.insert("A", "S");
        it.insert("F", "T"); // likely collides with A

        String artists = it.print("artist");
        assertTrue(artists.contains("|A|"));
        assertTrue(artists.contains("|F|"));
    }
     ...
     * Print empty tables
     */
    public void testPrintEmptyTables() throws Exception {
        it = new SongsDB();
        it.create(10, 32);
    ...
    /**
     * Count correctness after inserts
     */
    public void testCounts() throws Exception {
        ...
        String artists = it.print("artist");
        assertTrue(artists.contains("total artists: 3"));

        String songs = it.print("song");
        assertTrue(songs.contains("total songs: 3"));
    }


    /**
     * Remove always returns "does not exist"
     */
    public void testRemoveAlwaysNotFound() throws Exception {
        it = new SongsDB();
        it.create(10, 32);

        assertFuzzyEquals(
            "|X| does not exist in the Artist database",
            it.remove("artist", "X"));

        assertFuzzyEquals(
            "|Y| does not exist in the song database",
            it.remove("song", "Y"));
    }
